...
public class SubscriptionsListFragment extends EndlessListFragment<Post, FeedItem, PostsMultiAdapter> implements
	...
	public static SubscriptionsListFragment newInstance() {
		return new SubscriptionsListFragment();
	}

	@Override
	public SubscriptionsPresenter createPresenter() {
		return new SubscriptionsPresenter(this);
	}
	...
	public void onItemClick(Post post, int position) {
		PostActivity.startActivityForResult(SubscriptionsListFragment.this, post, false, position);
	}
		...
		if (UserInstance.getInstance().isLogged()) {
			return new StandardErrorView(getContext(), new View.OnClickListener() {
				...
				public void onClick(View v) {
					...
					getString(R.string.no_observations_yet), getString(R.string.observe_something), getString(R.string.refresh));
		} else {
